    ...
    <section class="container-main">
      <div style="text-align: center">
        <svg height="600" width="900">
          <image
            id="hand"
            xlink:href="https://github.com/avturin1980/GSAP/blob/main/C2L6/hand.png?raw=true"
            x="325"
            y="480"
          />
          <path
...
export default defineComponent({
  setup() {
    ...
    onMounted(() => {
      // モーションパスプラグインを使用するとパスに沿って任意のオブジェクトをアニメーション化できる
      gsap.registerPlugin(MotionPathPlugin);

      timeline = gsap
        .timeline({
          repeat: 2, // 繰り返しの数
          repeatDelay: 5, // 繰り返し間の遅延
          defaults: { duration: 12, ease: 'power1.inOut' }
        })
        .to('#hand', {
          motionPath: {
            path: '#path',
            align: '#path', // ターゲット画像を配置
            alignOrigin: [0.28, 0.08] // ターゲット画像を配置
          }
        })
